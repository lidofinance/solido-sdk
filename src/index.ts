...
export class SolidoSDK {
  ...
  constructor(cluster: SupportedClusters, connection: Connection, referrerId?: string) {
    ...
    if (cluster === 'devnet') {
      throw new ErrorWrapper({code: ERROR_CODES.UNSUPPORTED_CLUSTER});
    }
  ...
  public async signAndConfirmTransaction(
    ...
    try {
      ...
      throw new ErrorWrapper({error, code: ERROR_CODES.CANNOT_CONFIRM_TRANSACTION});
    }
  }
  ...
  public async stake(props: StakeProps) {
    ...
    if (wallet.publicKey === null) {
      throw new ErrorWrapper({code: ERROR_CODES.NO_PUBLIC_KEY});
    }
